...
def handle_connection(url: str) -> dict:
    ...
    if response.status_code != 200:
        print(url)
        print(f"[{response.status_code}]: {json.loads(response.text)['message']}")
    return response
...
def get_modules_list(sha: str) -> list:
...
def compare_versions(version_lower: str, version_higher: str) -> bool:
    """
    Checks if version_higher is higher than version_lower.
    ...
    Returns:
        bool: True if version_higher is higher than version_lower, False if equal or lower.
    """
...
def run_script():
    ...
    pull_request_commits_sha_list = get_pull_request_commits_sha_list(PULL_REQUEST_URL)
    for commit_sha, commit_name in pull_request_commits_sha_list:
        print(f"Commit: {commit_name}")
        modules_list = get_modules_list(commit_sha)
        for module in modules_list:
            module_version_from_commit = get_module_version_by_commit(commit_sha, module)
            module_version_from_branch = get_module_version_by_branch(module)
            print(f"{module}: pull_request: {module_version_from_commit}, {COMPARASION_BASE}: {module_version_from_branch}")
            is_pull_correct &= compare_versions(module_version_from_branch, module_version_from_commit)
    if is_pull_correct:
        print("ALLES GUT!")
    else:
